...
function bindUI(){
  ...
  // FAB: listeners + delegación global (además del onclick del HTML)
...
function renderOverview(){
  // guard rails: oculta cualquier overlay que pudiera quedar abierto
  ...
  const minW=300;
  let cols=Math.floor(W/(minW+pad)); cols=Math.max(1,Math.min(3,cols||1));
...
function doUndo(it){
  ...
  mode='overview'; renderOverview();  // <- volver a general
}
